...
//            $request = json_decode(json_encode($request->all()));
    ...
    public function orderDetails($request, $order_id)
    {
        ...
        $optionsList = ArrayHelper::map($selectedPlanOptions->toArray(), 'option', 'values');

        if (isset($optionsList['commission_online'])) {
            $total_commission =  ($total * $optionsList['commission_online']) / 100 ;

            $commissionLog = new OrderCommissionLog();
            $commissionLog->order_id = $order_id;
            $commissionLog->business_id = Auth::user()->business_id;
            $commissionLog->commission_percentage = $optionsList['commission_online'];
            $commissionLog->commission_value = $total_commission;
            $commissionLog->save();
        }
