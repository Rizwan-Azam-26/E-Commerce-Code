...
//        parent::__construct('product');
    ...
    public function uploadImage(Request $request)
    {
        ...
        if (!empty($request->file('images'))) {
            $rule = ['images.*' => [Rule::dimensions()->minHeight(400)->minWidth(360), 'max:2000', 'mimes:jpeg,png,jpg,gif,bmp']];
        }

        $validator = Validator::make($request->all(), $rule, [
            'images.*.dimensions' => 'Min image dimensions are 360(w) X 400(h)',
            'images.*.mimes' => 'Please upload only supported file types. (JPG, PNG, BMP)',
            'images.*.max' => 'Logo must be less than 2MB',
        ]);
