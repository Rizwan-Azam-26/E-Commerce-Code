            ...
            if (!empty(Auth::user()->business_id)) {
                ...
                if (plan_has_permission(['product-list', 'product-edit'])) {
                    $products = Product::whereNull('products.deleted_at')
                        ->selectRaw('products.title as name, products.id')
                        ->leftJoin('brands', 'brands.id','=','products.brand_id')
                        ->whereRaw(DB::raw('products.business_id = ' . Auth::user()->business_id . ''))
                        ->whereRaw(DB::raw('(products.title like "%' . $search . '%" or brands.title like "%' . $search . '%")'))
                        ->orderByRaw('products.created_at desc')->take(5)->get();
